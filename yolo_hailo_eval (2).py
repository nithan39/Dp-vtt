    ...
    def evaluate_hailo_npu(self):
        ...
        try:
            ...
            with VDevice(params) as target:
                ...
                quantized = (input_format == FormatType.UINT8)
                format_type = FormatType.UINT8 if quantized else FormatType.FLOAT32
                
                print(f"✓ Using quantized={quantized}, format_type={format_type}")
                
                # Create input/output vstream parameters
                input_vstreams_params = InputVStreamParams.make(network_group, quantized=quantized, format_type=format_type)
                output_vstreams_params = OutputVStreamParams.make(network_group, quantized=quantized, format_type=format_type)
