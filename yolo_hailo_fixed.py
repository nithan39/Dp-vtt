...
class YOLOEvaluator:
    ...
    def evaluate_hailo_npu(self):
        ...
        try:
            ...
            with VDevice(params) as target:
                ...
                frame_size = input_vstream_info.get_frame_size()
    ...
    def preprocess_for_hailo(self, image, input_vstream_info, frame_size):
        """
        ...
        # Ensure correct size
        expected_elements = frame_size // result.itemsize
        actual_elements = result.size
        
        if actual_elements != expected_elements:
            print(f"  Adjusting buffer: {actual_elements} elements -> {expected_elements} elements")
            # Flatten, adjust size, and reshape
            flat = result.flatten()
            if len(flat) > expected_elements:
                flat = flat[:expected_elements]
            elif len(flat) < expected_elements:
                flat = np.pad(flat, (0, expected_elements - len(flat)))
            result = flat.reshape(-1)
